...
contract HelloWorldTaskManagerSetup is Test {
    ...
    function createTask(TrafficGenerator memory generator, string memory taskName, string memory videoId, string memory taskContent, uint256 deadline) internal {
        ...
        helloWorldServiceManager.createNewTask(videoId, taskContent, deadline);
    }
    ...
    function respondToTask(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked("Hello, ", task.task_content));
...
contract CreateTask is HelloWorldTaskManagerSetup {
    ...
    function testCreateTask() public {
        string memory videoId = "1234567890";
        string memory taskContent = "TestTask";
        uint256 deadline = block.timestamp + 1 days;

        vm.prank(generator.key.addr);
        IHelloWorldServiceManager.Task memory newTask = sm.createNewTask(videoId, taskContent, deadline);
    }
}
...
contract RespondToTask is HelloWorldTaskManagerSetup {
    ...
    function testRespondToTask() public {
        ...
        bytes32 messageHash = keccak256(abi.encodePacked("Hello, ", videoId, taskContent));
